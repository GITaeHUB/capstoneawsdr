    ...
    void image_callback(const sensor_msgs::msg::Image::SharedPtr msg) {
        try {
            ...
            for (int i = 0; i < num_windows; ++i) {
                ...
                cv::Mat yellow_roi = yellow_mask(left_win);
                cv::Mat white_roi = white_mask(left_win);
                int yellow_count = cv::countNonZero(yellow_roi);
                int white_count = cv::countNonZero(white_roi);
                // RCLCPP_WARN(this->get_logger(), "[window %d] yellow: %d, white: %d", i,yellow_count, white_count);
            ...
            if (left_centers.size() > idx && right_centers.size() > idx) {
                ...
                // RCLCPP_WARN(this->get_logger(), "Insufficient lane data — usinging previous control");
                mid_x = mid_x_prev;
